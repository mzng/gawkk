class SearchController < ApplicationController
  ...
  def index
    ...
    @videos = Video.search(@q, :order => :posted_at, :sort_mode => 'posted_at DESC', :page => @page, :per_page => 5, :conditions => {:category_id => Category.allowed_on_front_page_ids}, :retry_stale => true)
  end
  ...
  def videos
    ...
    @videos = Video.search(@q, :order => :posted_at, :sort_mode => 'posted_at DESC', :page => @page, :per_page => @per_page, :conditions => {:category_id => Category.allowed_on_front_page_ids}, :retry_stale => true)
  end
