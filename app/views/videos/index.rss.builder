...
xml.rss(:version => "2.0", "xmlns:media" => 'http://search.yahoo.com/mrss/') {
  xml.channel{
    xml.title("Gawkk - All Categories - #{@popular ? 'Popular' : 'Newest'} Videos")
    ...
    @videos.each do |video|
      xml.item do
        ...
        xml.pubDate((@popular and video.promoted_at) ? video.promoted_at.strftime("%Y-%m-%dT%H:%M%z").insert(-3, ':') : video.posted_at.strftime("%Y-%m-%dT%H:%M%z").insert(-3, ':'))
